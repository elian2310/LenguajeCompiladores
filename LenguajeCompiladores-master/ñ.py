...
# Constantes
#######################################
from ast import Eq, expr, keyword
...
class PorNodo:
    def __init__(self, nombre_var_tok, inicio_valor_nodo, fin_valor_nodo, paso_valor_nodo, cuerpo_nodo ):
        self.nombre_var_tok = nombre_var_tok
        self.inicio_valor_nodo=inicio_valor_nodo
        self.fin_valor_nodo=fin_valor_nodo
        self.paso_valor_nodo=paso_valor_nodo
        self.cuerpo_nodo=cuerpo_nodo

        self.pos_start = self.nombre_var_tok.pos_start
        self.pos_end = self.cuerpo_nodo.pos_fin
...
class MientrasNodo:
    def __init__(self, condicion_nodo, cuerpo_nodo):
        ...
        self.pos_start = self.condicion_nodo.pos_start
...
####################################### 
    ...
    def por_expr(self):
        ...
        inicio_valor =res.registrar(self.expr())
        if res.error: return res
        ...
        fin_valor = res.registrar(self.expr())
        if res.error: return res
        ...
        if self.current_tok.iguala(PALCL, 'PASO'):
            ...
            paso_valor = res.registrar(self.expr())
            if res.error: return res
        else:
            paso_valor = None
        if not self.current_tok.iguala(PALCL,'LUEGO'):
            return res.fallo(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'THEN'"
            ))
        ...
        if res.error: return res 
        return res.correcto(PorNodo(name_var, inicio_valor, fin_valor, paso_valor,cuerpo))
...
class Interpretador:
    ...
    def visit_PorNodo(self, nodo, contexto):
        res = RuntimeResult()
        iniciar_valor=res.register(self.visit(nodo.iniciar_valor_nodo, contexto))
        if res.error: return res
        fin_valor=res.register(self.visit(nodo.fin_valor_nodo, contexto))
        if res.error: return res
        if nodo.paso_valor_nodo:
            paso_valor= res.register(self.visit(nodo.step_valor_nodo, contexto))
            if res.error: return res
        else:
            paso_valor=Numero(1)
        i=iniciar_valor.valor
        if paso_valor.valor>=0:
            condicion = lambda: i < fin_valor.valor
        else:
            condicion = lambda: i > fin_valor.valor
        while condicion():
            contexto.symbol_table.set(nodo.name_var.valor, Numero(i))
            i+= paso_valor.valor

            res.register(self.visit(nodo.cuerpo_nodo, contexto))
            if res.error: return res
        return res.correcto(None)
